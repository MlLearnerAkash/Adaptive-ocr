...
class BaseHTR(object):
    ...
    def init_train_params(self):
        if self.opt.adam:
            self.optimizer = optim.Adam(self.parameters, lr=self.opt.lr, betas=(self.opt.beta1, 0.999), weight_decay = 1e-5)
        ...
        if self.opt.StepLR:
            ...
            self.scheduler = None
        # scheduler = torch.optim.lr_scheduler.CyclicLR(self.optimizer, base_lr=0.00001, max_lr=0.001,
