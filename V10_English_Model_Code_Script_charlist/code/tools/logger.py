...
class Writer(object):
	...
	def update_trloss(self, value):
		...
		if self.use_tb:
			self.tb_writer.add_scalar('Loss/train', self.tr_loss, self.epoch)


	def update_valloss(self, value1, value2):
		self.val_loss = value1
		self.latest_wer = value2
		self.latest_wrr = 100-value2
		self.update()

		self.log_obj.write('Validation loss: {0}, WER: {1} \n'.format(
						   self.val_loss, self.latest_wer))
		if self.use_tb:
			print(">>>>>>>>>>>>loss vs wrr" )
			
			self.tb_writer.add_scalar('Loss/val', self.val_loss, self.epoch)
			self.tb_writer.add_scalar('WER/val', self.latest_wer, self.epoch)
			self.tb_writer.add_scalar('WRR/Val', self.latest_wrr, self.epoch)
	...
	def update_best_er(self, value1, value2):
		...
		if self.use_tb:
			self.tb_writer.add_text('Best Model', str(self.best_wer))
